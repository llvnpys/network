/*
    서버에서 채팅방 관리 및 메시지 전송, 파일 송수신을 담당하는 클래스입니다.
    클라이언트의 요청을 처리하고, 채팅방 생성, 참여, 상태 확인, 퇴장 및 파일 업로드와 다운로드 기능을 제공합니다.
    각 클라이언트의 소켓을 관리하고, 메시지를 브로드캐스트하여 채팅방 내에서 소통할 수 있게 합니다.

    주요 기능:
    - create(): 새로운 채팅방을 생성하고 클라이언트를 추가합니다.
    - join(): 클라이언트를 기존 채팅방에 추가합니다.
    - status(): 클라이언트가 속한 채팅방의 정보를 출력합니다.
    - exit(): 클라이언트가 채팅방을 나가며, 채팅방에 남은 클라이언트가 없으면 채팅방을 삭제합니다.
    - sendMessage(): 클라이언트로부터 받은 메시지를 같은 채팅방의 다른 클라이언트들에게 브로드캐스트합니다.
    - put(): 클라이언트로부터 파일을 수신하고 서버에 저장합니다.
    - get(): 서버에 저장된 파일을 클라이언트에게 전송합니다.
*/
